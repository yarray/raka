...
function activate(context) {
    ...
    const raskProvider = vscode.languages.registerCompletionItemProvider('ruby', {
        provideCompletionItems(document, position) {
            ...
            return methods.length > 0
                ? methods.map((m) => {
                    let item = new vscode.CompletionItem(m[0], vscode.CompletionItemKind.Method);
                    item.detail = m[1];
    ...
    const autoVarProvider = vscode.languages.registerCompletionItemProvider('ruby', {
        provideCompletionItems(document, position) {
            ...
            return vars
                .map((v) => {
                let item = new vscode.CompletionItem(v[0], vscode.CompletionItemKind.Variable);
                item.documentation = new vscode.MarkdownString(v[1]);
                return item;
            })
                .concat(parenVars.map((v) => {
                let item = new vscode.CompletionItem(v[0], vscode.CompletionItemKind.Variable);
                item.insertText = `(${v[0]})`;
                item.documentation = new vscode.MarkdownString(v[1]);
                return item;
